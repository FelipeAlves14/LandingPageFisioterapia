  ...
  return (
    <header className="flex flex-col sm:flex-row items-center justify-between sticky gap-4 sm:gap-0">
      <div className="bg-[url(/fundo-marca.png)] bg-no-repeat p-2 sm:p-4 w-full sm:w-auto">
        <img
          alt="Walterclay Abreu"
          src={marca}
          className="max-w-[120px] sm:max-w-none"
        />
      </div>
      <div className="flex flex-wrap justify-center text-(--option-color) w-full sm:w-full gap-2 sm:gap-0">
        <Button
          anchor="#formacoes"
      ...
      <div className="flex justify-end not-sm:hidden">
        <Button contact className="px-2 py-1 m-1 w-36" />
      </div>
    </header>
  );
}
